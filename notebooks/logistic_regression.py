...
def pipeline(DROP_NAN_THRESHOLD, CAT_NUM_THRESHOLD, DROP_CORR_THRESHOLD, BUILD_POLY, BUILD_LOG, BUILD_RATIOS, MAX_ITERS, GAMMA, k_indices, x_train, y_train):
    # drop_columns
...
with open('logistic_regression.csv', 'a', newline='') as csvfile:
    ...
    for _ in range(combinations):
        ...
        # call pipleline
        shape, eval_results_train, eval_results_test = pipeline(DROP_NAN_THRESHOLD, CAT_NUM_THRESHOLD, DROP_CORR_THRESHOLD, BUILD_POLY, BUILD_LOG, BUILD_RATIOS, int(MAX_ITERS), GAMMA, k_indices, x_train, y_train)
        # write to csv
        writer.writerow({
            "x_train_shape": shape,
